  ...
  const processTopPools = (allPools: Pool[]): PoolColumnDataType[] => {
    const topPools = allPools.map((pool, index) => {
      ...
      const riskChecklist: RiskChecklist = {
        lowPoolTVL: totalValueLockedUSD < 100000,
        lowPoolVolume: dailyVolumePerTVL < 0.1,
        highPriceVolatility: priceVolatility24HPercentage > 10,
        lowToken0TVL: Number(pool.token0.totalValueLockedUSD) < 250000,
        lowToken0PoolCount: pool.token0.poolCount < 3,
        lowToken1TVL: Number(pool.token1.totalValueLockedUSD) < 250000,
        lowToken1PoolCount: pool.token1.poolCount < 3,
      };
      const riskChecklistCount = Object.values(riskChecklist).filter(
        (v) => v === true
      ).length;
      let risk = Risk.SAFE;
      if (riskChecklistCount >= 3) risk = Risk.LOW_RISK;
      if (riskChecklistCount >= 6) risk = Risk.HIGH_RISK;
  ...
  return (
    <Container>
      <WrappedHeader>
        ...
        <Total>Displaying {pools.length} pools</Total>
      </WrappedHeader>
