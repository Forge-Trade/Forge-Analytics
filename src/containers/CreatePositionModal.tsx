...
const InstructionSection = ({ amount0, amount1 }: InstructionSectionProps) => {
  ...
  const isNative = (token: TokenType | null) => {
    ...
    if (platform === "evmos" && token.symbol === "EVMOS") {
      return true;
    }
      ...
      <Stepper>
        ...
        <li>
          ...
          <div className="step-content" style={{ marginBottom: 0 }}>
            ...
            <div>
              <a
                href={`https://app.forge.trade/#/add/${
                  isNative(state.token1) ? "EVMOS" : state.token1?.id
                }/${isNative(state.token0) ? "EVMOS" : state.token0?.id}/${
                  state.pool?.feeTier
                }${state.isFullRange ? "" : `?minPrice=${Pl}&maxPrice=${Pu}`}`}
                target="_blank"
              >
                <PrimaryButton style={{ width: "100%" }}>
                  Create Position on Forge
                </PrimaryButton>
              </a>
            </div>
          </div>
