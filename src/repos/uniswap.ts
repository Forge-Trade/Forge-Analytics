...
const _processTokenInfo = (token: Token) => {
  ...
  if (token.name === "Wrapped Evmos") {
    token.name = "Evmos Token";
    token.symbol = "WEVMOS";
  }
...
export const getPools = async (
  ...
  try {
    const res = await _queryUniswap(`{
      pools (first: 500, orderBy: totalValueLockedUSD, orderDirection: desc, where: {liquidity_gt: 0, totalValueLockedUSD_gte: ${totalValueLockedUSD_gte}, volumeUSD_gte: ${volumeUSD_gte}}) {
        id
        token0 {
          id
        }
